...
0. **広告の ATT 対応はリリース時に App Store Connect の手動更新が要る**: `services/adService.ts` は起動時に ATT を求め IDFA を広告に使う（[decisions/0007](./decisions/0007-att-and-ad-targeting.md)）。提出前に、アプリのプライバシー（デバイス ID → サードパーティ広告 → トラッキングあり）と提出時の IDFA 質問（「はい」）を更新すること。ゲーム系広告ネットワークのメディエーションは未導入。
